...
def compute_left_ventricle_volumes_single_plane(
    ...
    '''Computes the volumes of the left ventricle from single plane.
    
    '''
    diameters, step_size = _compute_diameters(pred,voxelspacing)
    diameters /= 1000
    step_size /= 1000

    # Estimate left ventricle volume from orthogonal disks
    lv_volume = np.sum(diameters * diameters) * step_size * np.pi / 4
