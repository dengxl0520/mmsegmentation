...
def draw(data, save_path):
    ...
    sns.lineplot(data=data['before'], linewidth=2, label='before')
    sns.lineplot(data=data['after'], linewidth=2, label='after')
...
def main():
    ...
    model_before = init_model(config=before_config_path, checkpoint=before_checkpoint_path)
    model_after = init_model(config=after_config_path, checkpoint=after_checkpoint_path)
    set_model_info([model_before, model_after])
    model_before.eval()
    model_after.eval()
    ...
    with torch.no_grad():
        ...
        for video_name in videos_list:
            ...
            while start < end and end - start > T:
                ...
                start = start + T
                data = preprare_data(video, video_info, model_before)
                results_before = model_before._forward(**data)[-1]
                results_after = model_after._forward(**data)[-1]
                pred_before = post_preprocess(results_before, threshold=threshold)
                pred_after = post_preprocess(results_after, threshold=threshold)
                before_list = np.concatenate((before_list, pred_before),axis=0)
                after_list = np.concatenate((after_list, pred_after),axis=0)
            x = [i for i in range(before_list.shape[0])]
            data = pd.DataFrame(np.stack((before_list,after_list)).transpose(1,0),x,columns=['before', 'after'])
