...
class STEchoHead(Mask2FormerHead):
    def __init__(self,
                loss_decode,
                 **kwargs):
        ...
        self.output = nn.Linear(feat_channels, self.num_classes)
        # self.output = nn.Conv2d(feat_channels, self.num_classes, kernel_size=1)
    ...
    def loss(self, x: Tuple[Tensor], batch_data_samples: SampleList,
        ...
        # loss
        # losses = self.loss_by_feat(all_mask_preds, batch_gt_instances, batch_img_metas)

        mask_features, multi_scale_memorys = self.pixel_decoder(x)

        x = mask_features.permute(0,2,3,1)
    ...
    def predict(self, x: Tuple[Tensor], batch_img_metas: List[dict],
        ...
        if 'pad_shape' in batch_img_metas[0]:
            size = batch_img_metas[0].get('pad_shape')
        else:
            size = batch_img_metas[0].get('img_shape')
    ...
    def loss_by_feat(self, all_mask_preds: Tensor,
                     batch_gt_instances: List[InstanceData],
                     batch_img_metas: List[dict]) -> Dict[str, Tensor]:
        num_dec_layers = len(all_mask_preds)
        batch_gt_instances_list = [ 
            batch_gt_instances for _ in range(num_dec_layers)
        ]
        img_metas_list = [batch_img_metas for _ in range(num_dec_layers)]
        losses_mask, losses_dice = multi_apply(
            self._loss_by_feat_single, all_mask_preds,
        ...
        num_dec_layer = 0
        for loss_mask_i, loss_dice_i in zip(losses_mask[:-1], losses_dice[:-1]):
